...
export class DropComponent implements OnInit {
  ...
  ngOnInit () {
    ...
    if (this.isMatchServeDrop(this.id)) {
      this.drop = servDropList.root.drop.find((drop: any) => drop['編號'] === this.id);
    } else {
      this.drop = dropList.root.drop.find((drop: any) => drop['編號'] === this.id);
    }
    const factor = +this.drop['factor'];
    const adv_factor = +this.drop['adv_factor'];
    let sum_prob = 0;
    for (let i = 1; i <= 20; i++) {
