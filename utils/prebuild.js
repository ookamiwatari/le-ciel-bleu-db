...
function parseUpdateIni (update_string) {
  return new Promise((resolve, reject) => {
    let update = update_string.split('[rebuildpak]\r\n\r\n')[1].split('\r\n[version]\r\n').map((version) => {
...
function downloadFiles (update) {
  ...
  return new Promise((resolve, reject) => {
    const files = update.filter((u) => u.version > process.env.LATEST_VERSION).map((u) => u.files).flat().filter((file) => file.path.indexOf('setting') !== -1 && file.name.indexOf('xml') !== -1);
    if (process.env.TEST_MODE === 'true') return resolve(files);
    files.forEach((file, i) => {
      const result = execSync(`wget -N ${process.env.UPDATE_URL}/${file.path}/${file.name}.pak`, {cwd: 'utils/download'});
    });
    const result = execSync(`wget -N ${process.env.UPDATE_URL}/update.ini`, {cwd: 'utils/download'});
...
function unpackFiles (files) {
  return new Promise((resolve, reject) => {
    ...
    files.forEach((file, i) => {
      try {
        const result = execSync(`cmd.exe /c unpacker.exe ../download/${file.name}.pak`, {cwd: 'utils/unpacker'});
      } catch (e) {
        //console.log('e', e);
      }
    });
...
function renameFiles (files) {
  return new Promise((resolve, reject) => {
    const { execSync } = require('child_process')
    files.forEach((file, i) => {
      const source = `unpacker/${file.path.toUpperCase()}/${file.name.slice(7).toUpperCase()}`;
...
function convertJson(files) {
  return new Promise((resolve, reject) => {
    ...
    ['item', 'drop', 'monster', 'magic', 'quest', 'quest_stepinfo', 'quest_steplist', 'pet', 'petattrib', 'serv_drop'].forEach((name) => {
      ...
      parseString(text, function (err, result) {
        for (let key in result.root) {
          result.root[key] = result.root[key].map((r) => r['$'] ? r['$'] : '');
        }
        fs.writeFileSync(`./src/assets/json/${name}.json`, JSON.stringify(result, null, '  '));
      });
    });
    resolve(files);
  })
